...
public class DifficultyManager {
    private static final String TAG = "MaramuresDifficultyManager";
    private static final String PREFS_NAME = "MaramuresDifficulty";
    ...
    public enum DifficultyLevel {
        ...
        DifficultyLevel(int timePerQuestion, float pointsMultiplier, int availableLifelines, String displayName, String description) {
    ...
    public DifficultyLevel calculateAdaptiveDifficulty(int correctAnswers, int totalQuestions, long averageTimePerQuestion) {
        if (totalQuestions < 5) {
            return DifficultyLevel.NORMAL;
        }
        float accuracy = (float) correctAnswers / totalQuestions;
        float timeRatio = (float) averageTimePerQuestion / 30000;
    ...
    public void updateDifficultyAfterGame(int correctAnswers, int totalQuestions, long totalTimeSpent) {
        ...
        DifficultyLevel newDifficulty = calculateAdaptiveDifficulty(totalCorrect, totalGames * 10, averageTime);
        prefs.edit()
    ...
    public String getPerformanceRecommendation() {
        ...
        if (totalGames == 0) {
            return "Bine ai venit în Maramureș! Începe cu nivelul Normal și vezi cum te descurci.";
        }
        float overallAccuracy = (float) totalCorrect / (totalGames * 10);
        if (overallAccuracy >= 0.9f) {
            return "Performanță excelentă! Încearcă nivelul Expert pentru o provocare mai mare.";
        } else if (overallAccuracy >= 0.7f) {
            return "Performanță bună! Continuă să exersezi pentru a ajunge la nivelul următor.";
        } else {
            return "Continuă să exersezi! Încearcă să te concentrezi pe categoriile cu care ai dificultăți.";
        }
    }
