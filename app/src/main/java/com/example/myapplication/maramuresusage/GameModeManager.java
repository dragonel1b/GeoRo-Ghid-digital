...
public class GameModeManager {
    public enum GameMode {
        CLASSIC("Quiz Clasic", 10, 30, false, "🎯", "Joc standard cu 10 întrebări și 30 secunde per întrebare"),
        LIGHTNING("Lightning Round", 20, 15, false, "⚡", "Răspunde rapid la 20 întrebări în 15 secunde fiecare"),
        MARATHON("Maraton", 50, 45, false, "🏃", "Test de rezistență cu 50 întrebări și 45 secunde per întrebare"),
        SURVIVAL("Supraviețuire", -1, 20, true, "💀", "Continuă până la prima greșeală - o singură viață!"),
        TIMED_CHALLENGE("Provocare Cronometrată", 15, 10, false, "⏰", "Cursă contra cronometru - 15 întrebări în 10 secunde"),
        CATEGORY_FOCUS("Focus Categorie", 15, 30, false, "📚", "Concentrează-te pe o singură categorie de întrebări"),
        MIXED_DIFFICULTY("Dificultate Mixtă", 12, 25, false, "🎲", "Întrebări de toate nivelurile într-un singur quiz"),
        BLITZ("Blitz", 30, 8, false, "💨", "Super rapid - 30 întrebări în 8 secunde fiecare!"),
        EXPERT_CHALLENGE("Provocarea Expertului", 8, 60, false, "🎓", "Doar întrebări expert cu timp extins pentru gândire");
        ...
        GameMode(String displayName, int questionCount, int timePerQuestion, boolean isEliminationMode, String emoji, String description) {
    ...
    public GameModeManager(Context context) {
        ...
        this.prefs = context.getSharedPreferences("MaramuresGameModePrefs", Context.MODE_PRIVATE);
    ...
    public List<EnhancedQuestionModel> filterQuestionsForGameMode(List<EnhancedQuestionModel> allQuestions) {
        ...
        if (currentGameMode.questionCount > 0 && filteredQuestions.size() > currentGameMode.questionCount) {
            filteredQuestions = filteredQuestions.subList(0, currentGameMode.questionCount);
        }
        return filteredQuestions;
    }
    private List<EnhancedQuestionModel> balanceDifficultyDistribution(List<EnhancedQuestionModel> questions) {
        List<EnhancedQuestionModel> balanced = new ArrayList<>();
        List<EnhancedQuestionModel> easy = questions.stream().filter(q -> q.getDifficulty() == EnhancedQuestionModel.Difficulty.EASY).collect(Collectors.toList());
        List<EnhancedQuestionModel> medium = questions.stream().filter(q -> q.getDifficulty() == EnhancedQuestionModel.Difficulty.MEDIUM).collect(Collectors.toList());
        List<EnhancedQuestionModel> hard = questions.stream().filter(q -> q.getDifficulty() == EnhancedQuestionModel.Difficulty.HARD).collect(Collectors.toList());
        List<EnhancedQuestionModel> expert = questions.stream().filter(q -> q.getDifficulty() == EnhancedQuestionModel.Difficulty.EXPERT).collect(Collectors.toList());
    ...
    public int calculateModeBonus(int baseScore, boolean isCorrect, long timeSpent) {
        ...
        int bonus = 0;
        int timePerQuestion = currentGameMode.timePerQuestion * 1000;
        switch (currentGameMode) {
            case LIGHTNING:
            case BLITZ:
                if (timeSpent < timePerQuestion * 0.5) {
                    bonus = (int)(baseScore * 0.5);
                } else if (timeSpent < timePerQuestion * 0.75) {
                    bonus = (int)(baseScore * 0.25);
                }
                break;
            case SURVIVAL:
    ...
    public int getTimePerQuestion() {
        return currentGameMode.timePerQuestion * 1000;
    }
