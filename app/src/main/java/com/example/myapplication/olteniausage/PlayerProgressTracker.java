...
/**
 * Tracker pentru progresul și performanța detaliată a jucătorului în Oltenia
 */
public class PlayerProgressTracker {
    private static final String TAG = "PlayerProgressTracker";
    private static final String PREFS_NAME = "OlteniaPlayerProgressTracking";
    ...
    private void generateLearningRecommendations(QuizStats stats) {
        ...
        for (CategoryStats categoryStats : stats.categoryStats.values()) {
            if (categoryStats.isWeak && categoryStats.questionsAnswered >= 3) {
                ...
                stats.recommendations.add(new LearningRecommendation(
                    ...
                    "Ai o acuratețe de " + String.format("%.1f", categoryStats.accuracy * 100) + 
                    "% în această categorie. Încearcă să exersezi mai mult.",
        ...
        // Recomandări pentru dificultăți
        for (DifficultyStats difficultyStats : stats.difficultyStats.values()) {
            if (!difficultyStats.isComfortable && difficultyStats.questionsAnswered >= 5) {
                stats.recommendations.add(new LearningRecommendation(
                    "Exersează " + difficultyStats.difficultyName,
                    "Încearcă să te îmbunătățești la nivelul " + difficultyStats.difficultyName,
                    "GENERAL",
                    difficultyStats.difficultyName,
                    4
                ));
            }
        }
        ...
        if (stats.overallAccuracy < 0.7f) {
            stats.recommendations.add(new LearningRecommendation(
                "Începe cu nivelul Normal",
                "Acuratețea ta generală este " + String.format("%.1f", stats.overallAccuracy * 100) + 
                "%. Încearcă să exersezi mai mult înainte de a trece la niveluri mai dificile.",
                "GENERAL",
                "NORMAL",
                3
            ));
    ...
    /**
     * Termină sesiunea și salvează statisticile
     */
    ...
    /**
     * Obține categoriile de întrebări recomandate
     */
    public List<String> getRecommendedQuestionCategories() {
        QuizStats stats = getCurrentStats();
        List<String> recommendations = new ArrayList<>();
        
        // Prioritizează subiectele slabe
        recommendations.addAll(stats.weakTopics);
        
        // Adaugă categorii cu performanță medie
        for (CategoryStats categoryStats : stats.categoryStats.values()) {
            if (!categoryStats.isWeak && !categoryStats.isStrong && 
                categoryStats.questionsAnswered >= 3) {
                recommendations.add(categoryStats.categoryName);
            }
        }
        
        // Adaugă categorii neîncercate
        for (EnhancedQuestionModel.Category category : EnhancedQuestionModel.Category.values()) {
            if (!stats.categoryStats.containsKey(category.name())) {
                recommendations.add(category.name());
            }
        }
    ...
    public List<String> checkForNewAchievements() {
        ...
        if (stats.overallAccuracy >= 0.95f && stats.totalQuestionsAnswered >= 20) {
            achievements.add("Maestru al Olteniei - 95%+ acuratețe");
        }
        
        if (stats.overallAccuracy >= 0.9f && stats.totalQuestionsAnswered >= 15) {
            achievements.add("Expert al Olteniei - 90%+ acuratețe");
        }
        
        // Realizări bazate pe serie
        if (stats.longestStreak >= 10) {
            achievements.add("Seria de Foc - 10+ răspunsuri consecutive");
        }
        
        if (stats.longestStreak >= 5) {
            achievements.add("Seria Bună - 5+ răspunsuri consecutive");
        }
        
        // Realizări bazate pe categorii
        for (CategoryStats categoryStats : stats.categoryStats.values()) {
            if (categoryStats.isStrong && categoryStats.questionsAnswered >= 10) {
                achievements.add("Maestru " + categoryStats.categoryName + 
                               " - Expert în această categorie");
            }
        }
    ...
    public String exportStatsAsJson() {
        return gson.toJson(currentSessionStats);
    }
} 